...
export function AppLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (isPos) setPosSidebarOpen(false);
  }, [isPos]);
  ...
  useEffect(() => {
    const minutes = isEnabled("advancedSecurity")
  ...
  return (
    <AppLayoutControlsProvider
      value={{
        sidebarOpen: isPos ? posSidebarOpen : !sidebarCollapsed,
        toggleSidebar: isPos
          ? () => setPosSidebarOpen((value) => !value)
    ...
    >
      <div className="h-screen overflow-hidden flex bg-canvas" dir="rtl">
        ...
        />
        {!isPos || posSidebarOpen ? (
          <div className="no-print">
            <Sidebar
              collapsed={isPos ? false : sidebarCollapsed}
              onClose={isPos ? () => setPosSidebarOpen(false) : undefined}
            />
          </div>
        ) : null}
        <div className="flex-1 min-w-0 min-h-0 flex flex-col">
          ...
          <main
            className={
              isPos
                ? "flex-1 min-h-0 overflow-hidden p-2.5"
                : "flex-1 min-h-0 overflow-x-hidden overflow-y-auto p-5 space-y-5"
            }
          >
